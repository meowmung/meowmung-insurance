...
# 데이터베이스에서 데이터 가져오기
def fetch_data_from_mysql(ti, **kwargs):
    ...
    ti.xcom_push(key='traindata_dog_df', value=df.to_json())  # DataFrame을 JSON으로 저장

# XCom에서 데이터 가져오기 및 X, y 분리
def get_data_from_xcom(ti, **kwargs):
